...
- **与原方案的差异**：
  ...
  - 自动测速组从 10 个减为 3 个：`♻️ 自动选择`、`纯净节点-USA`、`日韩台自动`。`🚀 节点选择` 和 AI 组的默认路径不变。
...
- **效果**：在 Linux 上用 sing-box 1.14.1 模拟 120 个节点、GOGC 与 iOS 相同（Go 内存上限按 50 MiB，iOS 为 40 MiB），启动时进程内存峰值约减少 4 MiB；节点更少时减少得更少。导出的 sing-box 配置约为原方案的一半。这不是 iOS 实测，实际效果以设备为准。
...
- 若不需要 `日韩台自动`，删掉该组可再少一个测速组（模拟中约再减少 2 MiB）。
